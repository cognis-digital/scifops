...
def to_console(r: ScanResult) -> str:
    ...
    for f in r.findings[:100]:
        lines.append(
            f"  {ICON[f.severity]} [{f.severity.value.upper():<10}] "
            f"{f.id:<14} {f.title}"
        )
        if f.location:
...
def to_markdown(r: ScanResult) -> str:
    ...
    for f in r.findings:
        out.append(
            ...
            f"{f.nist_800_53} | {f.disa_stig} | {f.mitre_attack} |"
        )
    out.append("")
    ...
    return json.dumps({
        "assessment-results": {
            ...
            "metadata": {
                ...
                "oscal-version": "1.1.0",
                "remarks": (
                    "PLACEHOLDER — operator must supply UUIDs, parties, "
                    "system-security-plan link"
                ),
            },
            "results": [{
